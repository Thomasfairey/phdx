...
def _render_fallback_writing_desk():
    ...
    with st.expander("📚 Reference Source / Context", expanded=False):
        ...
        if creds:
            ...
            if docs:
                ...
                sel = st.selectbox(
                    "Doc", list(opts.keys()), label_visibility="collapsed"
                )
                if (
                    sel != "-- Select --"
                    and opts[sel] != st.session_state["active_doc_id"]
                ):
                    st.session_state["active_doc_id"] = opts[sel]
                    st.session_state["loaded_doc_text"] = airlock.load_google_doc(
                        opts[sel]
                    )
        ...
        if prompt:
            with st.spinner("Generating..."):
                result = llm_gateway.generate_content(
                    prompt, "drafting", st.session_state.get("loaded_doc_text", "")
                )
    ...
    if st.session_state["generated_draft"]:
        ...
        st.download_button(
            "📥 Download", st.session_state["generated_draft"], "draft.md"
        )
...
def _render_fallback_auditor():
    ...
    try:
        ...
        audit_text = st.text_area(
            "Draft to audit", height=200, placeholder="Paste your thesis draft here..."
        )
