...
class TestLLMGateway:
    ...
    def test_route_task_heavy_lift_with_context(self):
        ...
        assert (
            _route_task("drafting", HEAVY_LIFT_THRESHOLD + 1, mock_models) == "context"
        )
    ...
    def test_route_task_heavy_lift_fallback(self):
        ...
        assert (
            _route_task("drafting", HEAVY_LIFT_THRESHOLD + 1, mock_models) == "writer"
        )
...
class TestAirlock:
    ...
    def test_get_auth_status_no_client_secret(self):
        ...
        with patch.object(airlock_module, "CLIENT_SECRET_PATH") as mock_path:
            ...
            assert (
                "oauth" in status["message"].lower()
                or "client_secret" in status["message"].lower()
            )
...
class TestDNAEngine:
    ...
    def test_analyze_hedging_frequency(self):
        ...
        assert (
            result["total_hedges"] > 0
        )  # Should detect "suggests", "perhaps", "might"
...
class TestAPIServer:
    ...
    def test_status_endpoint(self, client):
        ...
        with patch(
            "core.llm_gateway.get_available_models", return_value=["writer", "auditor"]
        ):
